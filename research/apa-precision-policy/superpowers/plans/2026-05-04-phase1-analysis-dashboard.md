# Phase 1 Analysis Dashboard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a single-script analysis that runs all 12 workloads through three precision strategies (uniform FP16, uniform INT8, evolved policy), generates five visualization sections, and serves a static HTML dashboard on port 3000.

**Architecture:** One Python script (`analysis/run_analysis.py`) with three phases: data collection (runs workloads, collects per-block stats/metrics), figure generation (matplotlib → PNG), and HTML assembly + serving. All existing computation logic lives in `common/` and is reused directly.

**Tech Stack:** Python, matplotlib, torch, stdlib (`http.server`, `base64`, `pathlib`)

---

### Task 1: Data Collection Module

**Files:**
- Create: `analysis/run_analysis.py`

This task builds the data collection phase — running all 12 workloads through three strategies and collecting the raw data needed for all five dashboard sections.

- [ ] **Step 1: Create analysis directory and script skeleton**

```bash
mkdir -p analysis/figures
```

Write `analysis/run_analysis.py`:
...
- [ ] **Step 2: Run script to verify data collection works**

Run: `cd /home/shadeform/flash-attention-5 && python -c "from analysis.run_analysis import collect_data; print('import OK')"`
...
```bash
git add analysis/run_analysis.py
git commit -m "feat(analysis): data collection for all workloads and strategies"
```

---

### Task 2: Figure Generation — Precision Heatmaps

**Files:**
- Modify: `analysis/run_analysis.py`

Add the hero visualization: a 3×4 grid of precision heatmaps showing which blocks get INT8 (blue) vs FP16 (red).

- [ ] **Step 1: Add heatmap generation function**

Append to `analysis/run_analysis.py`:
...
- [ ] **Step 2: Test heatmap generation with minimal run**

Run: `cd /home/shadeform/flash-attention-5 && python -c "
import sys, os
sys.path.insert(0, '.')
from analysis.run_analysis import collect_data, fig_precision_heatmaps
print('Collecting data...')
results = collect_data()
print('Generating heatmaps...')
fig_precision_heatmaps(results)
print('Done: analysis/figures/precision_heatmaps.png')
"`

Expected: PNG file created at `analysis/figures/precision_heatmaps.png`

- [ ] **Step 3: Commit**

```bash
git add analysis/run_analysis.py
git commit -m "feat(analysis): precision heatmap visualization"
```

---

### Task 3: Figure Generation — Compression Summary, Accuracy, Scaling

**Files:**
- Modify: `analysis/run_analysis.py`

Add the remaining three data-driven figures.

- [ ] **Step 1: Add compression summary bar chart**
...
- [ ] **Step 2: Add accuracy vs baselines grouped bar chart**
...
- [ ] **Step 3: Add sequence length scaling line chart**
...
```bash
git add analysis/run_analysis.py
git commit -m "feat(analysis): compression, accuracy, and scaling figures"
```

---

### Task 4: Figure Generation — Entropy Distribution

**Files:**
- Modify: `analysis/run_analysis.py`

Add the "why it works" two-panel figure.

- [ ] **Step 1: Add entropy distribution figure**
...
def fig_entropy_distribution(results):
    ...
    # Left panel: entropy histogram with threshold
    ...
    # Right panel: entropy vs precision decision scatter
    ent_int8 = [d["entropy"] for d in per_block_data if d["precision"] == "int8"]
    out_int8 = [d["has_outlier"] for d in per_block_data if d["precision"] == "int8"]
    ent_fp16 = [d["entropy"] for d in per_block_data if d["precision"] == "fp16"]
    out_fp16 = [d["has_outlier"] for d in per_block_data if d["precision"] == "fp16"]
...
```bash
git add analysis/run_analysis.py
git commit -m "feat(analysis): entropy distribution figure"
```

---

### Task 5: HTML Dashboard Assembly and Serving

**Files:**
- Modify: `analysis/run_analysis.py`

Build the HTML from the generated PNGs (embedded as base64) and serve it.

- [ ] **Step 1: Add HTML builder and main function**
...
def main():
    ...
    print("\n[1/3] Collecting data across 12 workloads × 3 strategies...")
    results = collect_data()
...
- [ ] **Step 2: Run end-to-end with --no-serve to verify**

Run: `cd /home/shadeform/flash-attention-5 && python analysis/run_analysis.py --no-serve`

Expected: All 12 workloads processed, 5 PNGs saved in `analysis/figures/`, `analysis/dashboard.html` generated.

- [ ] **Step 3: Verify all output files exist**

Run: `ls -la analysis/figures/ && ls -la analysis/dashboard.html`

Expected: 5 PNG files and 1 HTML file.

- [ ] **Step 4: Commit**

```bash
git add analysis/
git commit -m "feat(analysis): complete dashboard with HTML assembly and serving"
```

---

### Task 6: End-to-End Test and Serve

**Files:**
- No new files

- [ ] **Step 1: Run full pipeline and start server**

Run: `cd /home/shadeform/flash-attention-5 && python analysis/run_analysis.py --port 3000`

Expected: Dashboard generates and server starts. User can visit `http://<host>:3000/dashboard.html` in browser.

- [ ] **Step 2: Verify dashboard loads**

Run (in separate terminal): `curl -s http://localhost:3000/dashboard.html | head -5`
...
```bash
git add analysis/
git commit -m "fix(analysis): polish dashboard output"
```
